...
class AmfphpMonitorService {
    ...
    public function getData($flush){
        if(!is_writable(self::$logPath) || !is_readable(self::$logPath)){
            throw new \amf\core\Exception('AmfphpMonitor does not have permission to read and write to log file: ' . self::$logPath);
        }
    ...
    public function flush(){
        ...
        if(file_put_contents(self::$logPath, "<?php exit();?>\n") === false){
            throw new \amf\core\Exception('AmfphpMonitor write file failed ' . self::$logPath);
        }
    }
}
