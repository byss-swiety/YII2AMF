 ...
 */
namespace amf\plugins\AmfphpVoConverter;

use amf\core\common\IVoConverter;
 ...
 * 
 * The explicit type marker is defined in \amf\core\amf\Constants
 * 
 ...
 *  
 * @see \amf\core\amf\Constants::FIELD_EXPLICIT_TYPE
 ...
 */
class AmfphpVoConverter implements IVoConverter {
    ...
    public function __construct(array $config = null) {
        ...
        $filterManager->addFilter(Gateway::FILTER_VO_CONVERTER, $this, 'filterVoConverter');
    ...
    public function filterDeserializedRequest($deserializedRequest) {
        $ret = null;
        if($this->scanEnabled){
            $ret = \amf\core\amf\Util::applyFunctionToContainedObjects($deserializedRequest, array($this, 'convertToTyped'));
        }
    ...
    public function filterDeserializedResponse($deserializedResponse) {
        $ret = null;
        if($this->scanEnabled){
            $ret = \amf\core\amf\Util::applyFunctionToContainedObjects($deserializedResponse, array($this, 'markExplicitType'));
        }
        if(class_exists('AmfphpMonitor', false)){
