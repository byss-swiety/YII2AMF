...
use amf\core\amf\Constants;
use amf\core\FilterManager;
use amf\core\amf\Handler;
use amf\core\Config;
/**
 *  includes
 *  */
use amf\plugins\AmfphpFlexMessaging\AcknowledgeMessage;
use amf\plugins\AmfphpFlexMessaging\ErrorMessage;
...
class AmfphpFlexMessaging {
    ...
    public function __construct(array $config = null) {
        FilterManager::getInstance()->addFilter(Handler::FILTER_AMF_REQUEST_MESSAGE_HANDLER, $this, 'filterAmfRequestMessageHandler');
        FilterManager::getInstance()->addFilter(Handler::FILTER_AMF_EXCEPTION_HANDLER, $this, 'filterAmfExceptionHandler');
        $this->clientUsesFlexMessaging = false;
        $this->returnErrorDetails = (isset($config[Config::CONFIG_RETURN_ERROR_DETAILS]) && $config[Config::CONFIG_RETURN_ERROR_DETAILS]);
    }
     ...
     * @return array
     */
    public function filterAmfRequestMessageHandler($handler, \amf\core\amf\Message $requestMessage) {
     ...
     */
    public function handleRequestMessage(\amf\core\amf\Message $requestMessage, \amf\core\common\ServiceRouter $serviceRouter) {
        $explicitTypeField = Constants::FIELD_EXPLICIT_TYPE;
        ...
        if ($messageType == self::FLEX_TYPE_COMMAND_MESSAGE) {
            ...
            return new \amf\core\amf\Message($requestMessage->responseUri . Constants::CLIENT_SUCCESS_METHOD, null, $acknowledge);
        }
        ...
        throw new \amf\core\Exception('unrecognized flex message');
    }
    ...
    public function generateErrorResponse(Exception $exception) {
        $error = new \amf\plugins\AmfphpFlexMessaging\ErrorMessage($this->lastFlexMessageId);
        ...
        $errorMessage = new \amf\core\amf\Message($this->lastFlexMessageResponseUri . Constants::CLIENT_FAILURE_METHOD, null, $error);
