...
class Deserializer implements IDeserializer {
    ...
    public function deserialize(array $getData, array $postData, $rawPostData) {
        ...
        $this->currentByte = 0;
        $this->deserializedPacket = new \amf\core\amf\Packet();
    ...
    protected function readHeaders() {
        ...
        //
            ...
            throw new amf\core\Exception('Malformed amf Packet, connection may have dropped');
        }
        if ($secondByte == 3) {
            $this->deserializedPacket->amfVersion = \amf\core\amf\Constants::AMF3_ENCODING;
        }
        ...
        while ($this->headersLeftToProcess--) { // loop over all of the header elements
            ...
            $header = new \amf\core\amf\Header($name, $required, $content);
    ...
    protected function readMessages() {
        ...
        while ($this->messagesLeftToProcess--) { // loop over all of the Message elements
            ...
            $message = new Message($target, $response, $data);
    ...
    public function readData($type) {
        switch ($type) {
            ...
            case 5: // null
                return null;
            case 6: // undefined
                return new \amf\core\amf\types\Undefined();
            ...
            default: // unknown case
                throw new \amf\core\Exception("Found unhandled type with code: $type");
                exit();
                break;
        }
        return $data;
     ...
     * to the object \amf\core\amf\Constants::FIELD_EXPLICIT_TYPE.  This property will be overwritten if it existed within the class already.
     *
    ...
    public function readAmf3Data() {
        ...
        switch ($type) {
            ...
            case 0x11 :
                throw new \amf\core\Exception('dictionaries not supported, as it is not possible to use an object as array key in PHP ');
            default:
                throw new \amf\core\Exception('undefined Amf3 type encountered: ' . $type);
        }
    }
    ...
    /**
     * read amf 3 date
     * @return boolean|\\amf\core\amf\types\Date
     * @throws \amf\core\Exception
     */
    protected function readAmf3Date() {
        ...
        if (($firstInt & 0x01) == 0) {
            ...
            if ($firstInt >= count($this->storedObjects)) {
                throw new \amf\core\Exception('Undefined date reference: ' . $firstInt);
                return false;
            }
    ...
    protected function readAmf3String() {
        ...
        if (($strref & 0x01) == 0) {
            ...
            if ($strref >= count($this->storedStrings)) {
                throw new \amf\core\Exception('Undefined string reference: ' . $strref, E_USER_ERROR);
                return false;
            }
    ...
    protected function readAmf3Xml() {
        ...
        return new \amf\core\amf\types\Xml($xml);
    }
    ...
    protected function readAmf3XmlDocument() {
        ...
        return new \amf\core\amf\types\XmlDocument($xml);
    }

    /**
     * read amf 3 byte array
     * @return \amf\core\amf\types\ByteArray
     */
    protected function readAmf3ByteArray() {
        ...
        $handle = $handle >> 1;
        if ($inline) {
            $ba = new \amf\core\amf\types\ByteArray($this->readBuffer($handle));
    ...
    protected function readAmf3Object() {
        ...
        if ($classDefinition['externalizable']) {
            if (($typeIdentifier == 'flex.messaging.io.ArrayCollection') || ($typeIdentifier == 'flex.messaging.io.ObjectProxy')) {
                ...
                $externalizedDataField = \amf\core\amf\Constants::FIELD_EXTERNALIZED_DATA;
    ...
    protected function readAmf3Vector($type) {
        ...
        $handle = $handle >> 1;
        if ($inline) {
            $vector = new \amf\core\amf\types\Vector();
            $vector->type = $type;
            ...
            if ($type === \amf\core\amf\types\Vector::VECTOR_OBJECT) {
                ...
                switch ($type) {
                    case \amf\core\amf\types\Vector::VECTOR_INT : ;
                        $length = 4;
                        $format = "ival";
                        break;
                    case \amf\core\amf\types\Vector::VECTOR_UINT :;
                        $length = 4;
                        $format = "Ival";
                        break;
                    case \amf\core\amf\types\Vector::VECTOR_DOUBLE : ;
                        $length = 8;
                        $format = "dval";
                        break;
                }
