 ...
 */
class Serializer implements \amf\core\common\ISerializer {
    ...
    protected function writeDouble($d) {
        ...
        if (\amf\core\amf\Util::isSystemBigEndian()) { // if we are a big-endian processor
            $r = strrev($b);
     ...
     * @param \amf\core\amf\types\Xml $d
     */
    protected function writeXML(\amf\core\amf\types\Xml $d) {
     ...
     *
     * @param \amf\core\amf\types\Date $d The date value
     */
    protected function writeDate(\amf\core\amf\types\Date $d) {
        $this->writeByte(0x0B);
     ...
     * To accomplish this we just blanket grab all of the object vars with get_object_vars, minus the \amf\core\amf\Constants::FIELD_EXPLICIT_TYPE field, whiuch is used as class name
     *
     * @param object $d The object to serialize the properties. The deserializer looks for \amf\core\amf\Constants::FIELD_EXPLICIT_TYPE on this object and writes it as the class name.
     */
    protected function writeTypedObject($d) {
        ...
        if (!$className) {
            throw new \amf\core\Exception(\amf\core\amf\Constants::FIELD_EXPLICIT_TYPE . ' not found on a object that is to be sent as typed. ' . print_r($d, true));
        }
    ...
    protected function writeData($d) {
        if ($this->packet->amfVersion == \amf\core\amf\Constants::AMF3_ENCODING) { //amf3 data. This is most often, so it's has been moved to the top to be first
            $this->writeByte(0x11);
            ...
            $this->writeNull();
            return;
        } elseif ($d instanceof \amf\core\amf\types\Undefined) {
            $this->writeUndefined();
            return;
        } elseif (is_array($d)) { // array
            $this->writeArrayOrObject($d);
            return;
        } elseif ($d instanceof \amf\core\amf\types\Date) { // date
            $this->writeDate($d);
            return;
        } elseif ($d instanceof \amf\core\amf\types\Xml) { // Xml (note, no XmlDoc in AMF0)
            $this->writeXML($d);
            return;
        } elseif (is_object($d)) {
            if ($this->voConverter) {
        ...
        throw new \amf\core\Exception("couldn't write data " . print_r($d));
    }
    ...
    protected function writeAmf3Data($d) {
        if (is_int($d)) { //int
            ...
            $this->writeAmf3Null();
            return;
        } elseif ($d instanceof \amf\core\amf\types\Undefined) { // undefined
            ...
            return;
        } elseif ($d instanceof \amf\core\amf\types\ByteArray) { //byte array
            $this->writeAmf3ByteArray($d);
            return;
        } elseif ($d instanceof \amf\core\amf\types\Xml) { // Xml
            $this->writeAmf3Xml($d);
            return;
        } elseif ($d instanceof \amf\core\amf\types\XmlDocument) { // XmlDoc
            $this->writeAmf3XmlDocument($d);
            return;
        } elseif ($d instanceof \amf\core\amf\types\Vector) {
        ...
        throw new \amf\core\Exception("couldn't write object " . print_r($d, false));
    }
    ...
    /**
     * write \amf\core\amf\types\Xml in amf3
     * @param \amf\core\amf\types\Xml $d
     */
    protected function writeAmf3Xml(\amf\core\amf\types\Xml $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x0B);
        $this->writeAmf3String($d);
    }
    ...
    /**
     * write \amf\core\amf\types\XmlDocument in amf3
     * @param \amf\core\amf\types\XmlDocument $d
     */
    protected function writeAmf3XmlDocument(\amf\core\amf\types\XmlDocument $d) {
        $d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
        $this->writeByte(0x07);
        $this->writeAmf3String($d);
    }
    ...
    /**
     * write \amf\core\amf\types\Date in amf 3
     * @param \amf\core\amf\types\Date $d
     */
    protected function writeAmf3Date(\amf\core\amf\types\Date $d) {
        $this->writeByte(0x08);
        $this->writeAmf3Int(1);
        $this->writeDouble($d->timeStamp);
    }
    ...
    /**
     * write \amf\core\amf\types\ByteArray in amf3
     * @param \amf\core\amf\types\ByteArray $d
     */
    protected function writeAmf3ByteArray(\amf\core\amf\types\ByteArray $d) {
        $this->writeByte(0x0C);
        $data = $d->data;
    ...
    protected function handleReference(&$obj, array &$references) {
        ...
        if ($key !== false) {
            ...
            if ($this->packet->amfVersion == \amf\core\amf\Constants::AMF0_ENCODING) {
    ...
    /**
     * write vector
     * @param \amf\core\amf\types\Vector $d 
     */
    protected function writeAMF3Vector(\amf\core\amf\types\Vector $d) {
        ...
        if ($d->type === \amf\core\amf\types\Vector::VECTOR_OBJECT) {
            ...
            if ($d->type == \amf\core\amf\types\Vector::VECTOR_INT) {
                $className = "i";
            } elseif ($d->type == \amf\core\amf\types\Vector::VECTOR_UINT) {
                $className = "I";
            } elseif ($d->type == \amf\core\amf\types\Vector::VECTOR_DOUBLE) {
                $className = "d";
            }
    ...
    function writeAmf3VectorValue($value, $format) {
        ...
        if (\amf\core\amf\Util::isSystemBigEndian()) {
            $bytes = strrev($bytes);
        }
