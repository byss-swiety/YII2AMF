...
class ServiceRouter {
    ...
    public static function getServiceObjectStatically($serviceName, array $serviceFolders, array $serviceNames2ClassFindInfo){
        ...
        if (!$serviceObject) {
            throw new \amf\core\Exception("$serviceName service not found ");
        }
    ...
    public function executeServiceCall($serviceName, $methodName, array $parameters) {
        ...
        $serviceObject = \amf\core\FilterManager::getInstance()->callFilters(self::FILTER_SERVICE_OBJECT, $unfilteredServiceObject, $serviceName, $methodName, $parameters);
        ...
        if(method_exists($serviceObject, $methodName)){
            ...
            throw new \amf\core\Exception("method $methodName not found on $serviceName object ");
        }
        ...
        if($this->checkArgumentCount){
            ...
            if ($numberOfProvidedParameters < $numberOfRequiredParameters || $numberOfProvidedParameters > $numberOfParameters) {
                throw new \amf\core\Exception("Invalid number of parameters for method $methodName in service $serviceName : $numberOfRequiredParameters  required, $numberOfParameters total, $numberOfProvidedParameters provided");
            }      
        }
        if($isStaticMethod){
